...
(async () => {
  ...
  try {
    ...
    const mainTalk = fs.readFileSync(path.join(__dirname, '../src/MainTalk.jsx'), 'utf8');
    ...
    const energyOrigins = fs.readFileSync(path.join(__dirname, '../src/components/JapanEnergyOrigins.jsx'), 'utf8');
    ...
    assert.match(energyOrigins, /sceneLayer=\{\{ view, layers \}\}/);
    assert.match(energyOrigins, /Number\.isFinite\(step\) \? step : 0/);
    assert.match(energyOrigins, /const HORMUZ_VIEW = \{ longitude: 98, latitude: 22, zoom: 2\.6, pitch: 30, bearing: 0 \}/);
    assert.match(energyOrigins, /StepBridge count=\{4\}/);
    assert.match(energyOrigins, /activeStep === 3 \? ORIGIN_OVERVIEW_VIEW : activeStep === 4 \? HORMUZ_FOCUS_VIEW : HORMUZ_VIEW/);
    assert.match(energyOrigins, /activeStep === 4 \? HORMUZ_FOCUS_VIEW/);
    assert.match(keynote, /defaultTransition, fadeTransition/);
    assert.match(keynote, /<Deck theme=\{spectacleTheme\} template=\{template\} transition=\{defaultTransition\} backdropStyle=\{\{ backgroundColor: 'var\(--color-washi-paper\)' \}\}>/);
    assert.match(mainTalk, /<Deck theme=\{spectacleTheme\} template=\{template\} backdropStyle=\{\{ backgroundColor: 'var\(--color-washi-paper\)' \}\}>/);
    assert.match(keynote, /<Slide backgroundColor="var\(--color-washi-paper\)" padding="0" transition=\{fadeTransition\}>\s*<JapanEnergyOrigins/);
    assert.match(keynote, /<Slide backgroundColor=\{bg\} padding="0" transition=\{fadeTransition\}>\s*<style>.*<StepBridge count=\{4\}>/s);
    assert.match(keynote, /data-testid="hormuz-map-handoff-fade"/);
    assert.match(keynote, /animation: hormuzMapHandoff 1000ms ease-in-out forwards/);
    assert.match(gridAtlas, /useState\(\(\) => initialView\(sceneLayer, routeLayer\)\)/);
    assert.match(gridAtlas, /const routeDuration = routeLayer\?\.duration \?\? 2200/);
    assert.match(gridAtlas, /const routeDelay = routeLayer\?\.delay \?\? 0/);
    assert.match(gridAtlas, /const targetView = routeLayer\?\.targetView \?\? sceneLayer\?\.view/);
    assert.match(gridAtlas, /transitionDuration: routeDuration/);
    assert.match(keynote, /routeLayer=\{\{ points: HORMUZ_ROUTE, view: HORMUZ_FOCUS_VIEW, targetView: JAPAN_VIEW, duration: 12000, delay: 1000, followShip: true, ship: \{ mesh: '\/models\/cargo-ship\.obj'/);
    assert.match(gridAtlas, /SimpleMeshLayer/);
    assert.match(gridAtlas, /OBJLoader/);
    assert.match(gridAtlas, /atlas-route-ship/);
    assert.match(gridAtlas, /const routePosition = \(points, progress\)/);
    assert.match(gridAtlas, /const progressRef = useRef\(0\)/);
    assert.match(gridAtlas, /progressRef\.current = progress/);
    assert.match(gridAtlas, /layers\(progressRef\.current\)/);
