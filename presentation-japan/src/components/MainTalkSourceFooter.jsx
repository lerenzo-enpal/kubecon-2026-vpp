...
export function MainTalkSourceFooter({ evidence, detailUrl, caseNote, compact = false }) {
  ...
  return (
    <footer data-testid="main-talk-source-footer" style={{ position: 'absolute', bottom: 22, left: 36, color: 'var(--color-muted)', fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.02em' }}>
      ...
      <span>{compact ? evidence.reference : `Source: ${evidence.sourceLabel} · ${evidence.reference || evidence.sourceYear}`}</span>
      {!compact && <span> · {evidence.notes}</span>}
      {!compact && researchUrl && <a href={researchUrl} style={{ color: 'inherit' }}> · Research notes</a>}
    </footer>
  );
}
