...
export function JapanGridAtlas({ height = 580, step = 0, preset = () => ({}), liveData = {}, variant = 'dark' }) {
  ...
  const layers = useMemo(() => [
    ...
    active.plants && new ScatterplotLayer({ id: 'atlas-plants', data: ATLAS_FEATURES.plants, getPosition: (d) => d.position, getRadius: 19000, radiusUnits: 'meters', getFillColor: (d) => PLANT_COLORS[d.fuel], getLineColor: [237, 233, 254, 255], lineWidthMinPixels: 2, stroked: true, pickable: true }),
  ...
  return <div data-testid="japan-grid-atlas" data-variant={variant} style={{ position: 'relative', width: '100%', height, background: variant === 'washi' ? 'var(--color-washi-paper)' : 'var(--color-bg)' }}>
    <DeckGL viewState={viewState} onViewStateChange={({ viewState: next }) => setViewState(next)} controller={true} layers={layers} getTooltip={({ object }) => object && (object.name ? { text: `${object.name}\n${object.fuel} · ${object.capacity}` } : { text: object.label || object.name })} style={{ position: 'absolute', inset: 0 }}>
