...
export function JapanGridAtlas({ height = '100%', step = 0, preset = () => ({}), liveData = {}, variant = 'dark', plantMarkerSize = 10 }) {
  ...
  }, [active.transmission, isSlideActive]);
  ...
  return <div data-testid="japan-grid-atlas" data-variant={variant} style={{ position: 'relative', width: '100%', height, background: variant === 'washi' ? 'var(--color-washi-paper)' : 'var(--color-bg)' }}>
    {isSlideActive && <DeckGL viewState={viewState} onViewStateChange={({ viewState: next }) => setViewState(next)} controller={true} layers={layers} getTooltip={({ object }) => object && (object.name ? { text: `${object.name}\n${object.fuel} · ${object.capacity}` } : { text: object.label || object.name })} style={{ position: 'absolute', inset: 0 }}>
