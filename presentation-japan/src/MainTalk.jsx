...
export default function MainTalk() {
  ...
  return <><PresentationChrome theme={theme} cycleTheme={cycleTheme} locale={locale} setLocale={setLocale} /><Deck theme={spectacleTheme} template={template}>
    ...
    <Slide padding="0"><div style={{ height: '100%', position: 'relative' }}><Lazy><JapanGridMap height={580} /></Lazy><div style={{ position: 'absolute', top: 28, left: 38, maxWidth: 620, padding: '20px 24px', background: 'color-mix(in srgb, var(--color-bg) 88%, transparent)' }}><Title>Japan cannot borrow</Title><Body>An island system, split between 50 Hz and 60 Hz, must solve each imbalance at home.</Body><div style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-washi-alert)', fontSize: 18, letterSpacing: '0.08em' }}>15.3% FY2023 PRIMARY-ENERGY SELF-SUFFICIENCY</div></div><Source evidence={E.japanEnergy} /></div><Notes>Orientation: island system; frequency seam; low primary-energy self-sufficiency; little room when supply tightens.</Notes></Slide>

    <Slide {...page}><div style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-washi-solar)', letterSpacing: '0.16em' }}>PROOF 1</div><Title>Make renewables usable</Title><Body>Solar can produce abundant clean power at noon. Demand often arrives later. Flexibility closes that timing gap.</Body><Notes>Chapter promise: renewables need timing, not only generation.</Notes></Slide>
    <Slide padding="0"><StepBridge count={3}>{step => <DaylightFlexibilityScene mode="problem" step={step} />}</StepBridge><Notes>Follow one day: morning balance; noon surplus; evening ramp.</Notes></Slide>
