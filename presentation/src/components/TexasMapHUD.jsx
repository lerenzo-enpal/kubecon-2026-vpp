...
export default function TexasMapHUD({ width = 1024, height = 700, variant = 'hud' }) {
  ...
  // Step forward — advance one cascade step, fly camera to target
  const stepForward = () => {
    const next = mode === 'idle' ? 0 : stepIndex + 1;
    if (next >= CASCADE.length) return;
    setMode('stepping');
    setStepIndex(next);
    // Accumulate failures up to this step
    const nf = new Set();
    for (let i = 0; i <= next; i++) CASCADE[i].ids.forEach(id => nf.add(id));
    nf.delete('comanche');
    setFailed(nf);
    setActiveStep(next);
  ...
  return (
    <div style={{ position: 'relative', width, height, overflow: 'hidden', background: '#020408' }}>
      ...
      {/* ════════════════════════════════════════════════════════
          CINEMATIC VARIANT — Minimal chrome, dramatic center text
          ════════════════════════════════════════════════════════ */}
      {variant === 'cinematic' && (
        <>
          {/* ── Top-left: Frequency ── */}
