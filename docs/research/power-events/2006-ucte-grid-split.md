# UCTE European Grid Split -- 4 November 2006
...
| **Households affected** | ~15 million (~35 million people) |
...
| **Frequency extremes** | West: ~49.0 Hz (nadir); North-East: ~51.4 Hz (peak) |
| **Root cause** | Planned 380 kV line disconnection without adequate N-1 security analysis |
...
### 3.1 Non-Fulfillment of the N-1 Security Criterion
...
### 3.2 Insufficient Inter-TSO Coordination
...
Wind turbines in the Western island disconnected due to their under-frequency protection settings, which were designed to protect the turbines rather than support the grid. This is a critical detail:
...
### UCTE Recommendations (2007)

The 26 recommendations covered:
...
### Ems River Line Modification (2007)

The Conneforde-Diele pylons were raised from **84 meters to 110 meters**, eliminating the need to de-energize the line for ship passages -- removing the specific trigger scenario permanently.

### Third Energy Package (2009)

The EU's Third Energy Package for the Internal Energy Market, adopted in 2009, was significantly influenced by the 2006 disturbance:
...
Following ENTSO-E's establishment, binding European network codes were developed covering:
...
The 2006 grid split is a textbook case study for why distributed, fast-responding resources -- exactly what modern VPPs provide -- are critical for grid resilience.

### The Problem VPPs Address

In 2006, when the grid split:
1. **No distributed frequency response existed** -- load shedding was the only automated defense
2. **Wind generators made things worse** -- they disconnected rather than supporting frequency
3. **Recovery depended entirely on centralized TSO actions** -- manual resynchronization by a handful of operators
4. **17 GW of load was shed** -- blunt, indiscriminate disconnection of entire feeders
...
5. **Geographic distribution**: A VPP's resources are inherently distributed across the grid. Unlike a single large power plant, they cannot all be lost due to a single transmission line failure. This provides resilience against the exact type of cascading failure that occurred in 2006.
...
## 9. Comparison with Other Major Grid Events
